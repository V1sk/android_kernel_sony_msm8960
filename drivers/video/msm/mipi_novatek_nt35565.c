...
static int mipi_nt35565_ic_on_disp_off(struct msm_fb_data_type *mfd)
{
	...
	if (!dsi_data->panel_detecting) {
		...
		if (pctrl->display_init_cmds) {
			cmdreq.cmds = pctrl->display_init_cmds;
			cmdreq.cmds_cnt = pctrl->display_init_cmds_size;
...
static int mipi_nt35565_ic_on_disp_on(struct msm_fb_data_type *mfd)
{
	...
	if (!dsi_data->panel_detecting) {
		...
		if (dsi_data->eco_mode_on && pctrl->display_on_eco_cmds) {
			cmdreq.cmds = pctrl->display_on_eco_cmds;
			cmdreq.cmds_cnt = pctrl->display_on_eco_cmds_size;
			cmdreq.flags = CMD_REQ_COMMIT;
			cmdreq.rlen = 0;
			cmdreq.cb = NULL;
			mipi_dsi_cmdlist_put(&cmdreq);
			...
			cmdreq.cmds = pctrl->display_on_cmds;
			cmdreq.cmds_cnt = pctrl->display_on_cmds_size;
			cmdreq.flags = CMD_REQ_COMMIT;
			cmdreq.rlen = 0;
			cmdreq.cb = NULL;
			mipi_dsi_cmdlist_put(&cmdreq);
...
static int mipi_nt35565_disp_off(struct msm_fb_data_type *mfd)
{
	...
	if (!dsi_data->panel_detecting) {
		...
		cmdreq.cmds = dsi_data->panel->pctrl->display_off_cmds;
		cmdreq.cmds_cnt = dsi_data->panel->pctrl->display_off_cmds_size;
		cmdreq.flags = CMD_REQ_COMMIT;
		cmdreq.rlen = 0;
		cmdreq.cb = NULL;
		mipi_dsi_cmdlist_put(&cmdreq);
