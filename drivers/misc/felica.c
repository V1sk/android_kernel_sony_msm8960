...
static enum power_supply_property snfc_properties[] = {
	POWER_SUPPLY_PROP_PRESENT,
};

static int snfc_get_property(struct power_supply *psy,
			enum power_supply_property psp,
			union power_supply_propval *val)
{
	struct felica_dev *d = container_of(psy, struct felica_dev,
						power_supply);
	...
	case POWER_SUPPLY_PROP_PRESENT:
		...
		break;
	default:
		dev_err(d->dev, "%s: INVALID property\n", __func__);
		return -EINVAL;
	}
...
static int snfc_set_property(struct power_supply *psy,
			enum power_supply_property psp,
...
static int snfc_property_is_writeable(struct power_supply *psy,
			enum power_supply_property psp)
{
	struct felica_dev *d = container_of(psy, struct felica_dev,
						power_supply);
...
static int snfc_power_supply_probe_func(struct felica_dev *dev)
{
	dev_dbg(dev->dev, "%s\n", __func__);

	if (power_supply_register(dev->dev, &dev->power_supply)) {
		dev_err(dev->dev,
			"%s: Error. Cannot register SNFC POWER SUPPLY.\n",
			__func__);
		return -ENODEV;
	}
...
static void snfc_power_supply_remove_func(struct felica_dev *dev)
{
	dev_dbg(dev->dev, "%s\n", __func__);

	power_supply_unregister(&dev->power_supply);
}
#endif
...
static int felica_probe(struct platform_device *pdev)
{
	...
	dev->power_supply.name = "snfc";
	dev->power_supply.type = POWER_SUPPLY_TYPE_MAINS;
	dev->power_supply.properties = snfc_properties;
	dev->power_supply.num_properties =
		ARRAY_SIZE(snfc_properties);
	dev->power_supply.get_property = snfc_get_property;
	dev->power_supply.set_property = snfc_set_property;
	dev->power_supply.property_is_writeable = snfc_property_is_writeable;
#endif
	...
	ret = snfc_power_supply_probe_func(dev);
	if (ret) {
		dev_err(&pdev->dev, "%s: SNFC POWER SUPPLY probe failure\n",
				__func__);
		goto err_snfc_power_supply_probe;
	}
#endif
...
err_create_sysfs:
#ifdef CONFIG_SONY_FELICA_NFC_SUPPORT
	snfc_power_supply_remove_func(dev);
err_snfc_power_supply_probe:
