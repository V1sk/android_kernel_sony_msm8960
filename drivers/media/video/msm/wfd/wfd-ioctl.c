...
static struct msm_wfd_clk_info wfd_clk_info[] = {
	{"dfab_clk", 2147483647},
};
...
static int wfd_open(struct file *filp)
{
	...
	rc = msm_wfd_clk_ctrl(&wfd_dev->pdev->dev, wfd_clk_info,
			wfd_dev->wfd_clk, ARRAY_SIZE(wfd_clk_info), 1);
	if (rc)
		goto err_clk_ctrl;
...
err_mdp_open:
	msm_wfd_clk_ctrl(&wfd_dev->pdev->dev, wfd_clk_info,
			wfd_dev->wfd_clk, ARRAY_SIZE(wfd_clk_info), 0);
	kfree(inst);
err_clk_ctrl:
	mutex_lock(&wfd_dev->dev_lock);
